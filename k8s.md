...
Hosts: `mock.local` → mock-server, `playground.local` → spring-playground, `grafana.local` → lgtm:3000 (Grafana), `redis.local` → RedisInsight.
...
- http://redis.local:8080/          → RedisInsight; add `redis:6379` as the Redis connection
