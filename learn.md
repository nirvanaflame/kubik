...
- [Phase 3b: Spring Boot Starter Integrations](#phase-3b-spring-boot-starter-integrations)
...
### Phase 2: Micrometer Core — ~5 hours
...
- [ ] Understand `MeterRegistry` as the central hub and how Spring Boot wires one up
- [ ] Know every core meter type, when to use it, defaults, and tag semantics
- [ ] Build a project from scratch with `spring-boot-starter-actuator` + Prometheus on JDK 21+ / Spring Boot 4.1
- [ ] Produce custom metrics (Counter/Timer/Gauge) in real code — REST controller, service layer, async task

### Mermaid: Core Meter Types

```mermaid
graph TD
    REG["MeterRegistry"] --> T1[Counter]          // increments only; e.g. `orders.created`
    REG --> T2[Gauge]                            // point-in-time value; e.g. active connections
    REG --> T3[Timer]            // duration/latency; histogram auto-generated by Micrometer
    REG --> T4[LongTaskTimer]  // long-running tasks (e.g. `@Async` with no callback)
    REG --> T5[FunctionTimer]  // timer over a function's call result, sampling based

    T1 -.-> "increment() / increment(delta)"
    T2 -.-> "set(value)"
    T3 -.-> "record(Duration)"
    T4 -.-> "start() → sample()"
```
...
|---------|---------------|
| `MeterRegistry` bean | The only object you inject; every metric goes through one of these (CompositeMeterRegistry is the default in Spring Boot) |
| `@Timed`, `@Counted` annotations | Zero-code instrumentation on methods/REST APIs — but AOP proxy must be created (`open` class in Kotlin, or enable aspectJ auto-proxying) |
| **Tags** (key/value pairs) | Granularity — a tag value count × number of unique combinations = cardininality. Limit dynamic tags! |
| **Common tags** vs. **dynamic tags** | Common via `management.metrics.tags.*` config; dynamic via programmatic tagging with `MeterRegistryCustomizer` or `@Tag` |
| `Timer.start()` / manual stop pattern | For code paths you can't annotate (e.g., inside a library callback) — remember to `.stop(timer)` in finally blocks! |
| **Step Duration** (default 1 min) | How often stats are flushed to the registry; don't change unless Prometheus scrape interval matches |
| **`@MetricNames` / `MeterFilter`** | Rename meters post-collection, bind gauges, drop unwanted tags — used in Phase 6 cardinality control |
...
## Phase 3b: Spring Boot Starter Integrations — ~2 hours

This phase adds metrics collection **without** writing custom `MeterRegistry` calls for common components. Spring Boot auto-configuration already wires up Prometheus endpoints for things like Redis, JDBC connection pools, and HTTP client latency on the classpath. These exercises make those hidden features visible.

### Learning Objectives

- [ ] Understand what happens when you add `spring-boot-starter-data-redis` / `-webflux` / `-data-jpa` to `build.gradle.kts`: which auto-config classes activate, which **Actuator endpoints** and metric categories become live.
- [ ] Collect cache hit/miss metrics from Redis (or Caffeine) after starting the app — observe the new series without writing code.
- [ ] See the JDBC connection-pool Gauges come online when a DataSource is wired in via `spring-boot-starter-data-jpa`.

### Mermaid: Spring Boot Auto-config Layers That Activate Metric Exposure

```mermaid
graph LR
    Subgraphs["Starter / starter-* deps added to build.gradle.kts"] --> AC[Auto-Configuration]
    ac -- "ConditionalOnClass" --> RDBMS[Redis auto-config]
    ac -- "ConditionalOnMissingBean"  --> JPA[JPA auto-config]
    Subgraphs --- HTTPWEB
```

### Quick Checklist: What Each Starter Adds to Obs Layer
| Starter | Auto-activated Metric Categories (visible at `/actuator/prometheus`) |
|---------|---------------------------------------------------------------------|
| `spring-boot-starter-web`   | `http.server.requests`, `tomcat.` (pool) |
| `spring-boot-starter-data-redis`   | `jedis.pool.*`, `cache.*`, **Redis-specific counters** when cache abstraction is on |
| `spring-boot-starter-data-jpa`     | `hikaricp.connections.*`, `hibernate.statistic.*` (Hibernate metrics) |
| `micrometer-registry-prometheus`  | Prometheus `/actuator/prometheus` endpoint + `prometheus.` prefix meters |

### Mini Task 3b-1 → see [Training Tasks](#training-tasks-by-phase) — #P-3b.1
...
graph TB
    ...
    subgraph DataPlane["📡 Observability Pipeline — OTel Collector config is the single source of truth"]
        ...
        subgraph "Exporter"
            ZK_EXP[Zipkin Exporter]
            JAEGER_EXP[Jaeger Exporter]
        end
        
        subgraph "Ingest (optional)"
            DATADOG["Datadog"]
            GCFLOW["Grafana Cloud — OTLP / PMD"]
        end
    end
    ...
    COLLECTOR---JAEGER_R
    COLLECTOR --> ZK_EXP
    COLLECTOR --> JAEGER_EXP
    COLLECTOR --> DATADOG & GCFLOW

    subgraph Visualize["📊 Visualization"]
        GRAFANA["Grafana<br/>Dashboard / Explore"]
        JMES[Jaeger UI]
        ZK_UI[Zipkin Web UI]
...
### D2: Micrometer Core Interfaces (Spring Boot) — _fixed_
...
classDiagram
    class MeterRegistry {
        ...
        Double gauge(Number valueFunction, Tags tagArray)
    }

    class Counter {
        +increment() void
        +increment(long n) long
    }

    class Timer {
        +record(Duration d) double
        +start() TaggedStopWatch
    }
    ...
    MeterRegistry o-- Timer
    
    Note for reader: JvmGcMetrics and CacheAspectStatistics are runtime Bindings — they register MeterRegistryBinders(Gauge registry) at startup, producing jvm.gc.* / cache.cache.* meters automatically. See Spring Boot's @Bean MeterBinder pattern.
```

### D3: OpenTelemetry SDK Layers (Spring Boot)

```mermaid
graph TB
    subgraph Application["Your Spring Boot App"]
        CONTROLLER["@RestController / WebFlux Handler"]
        REDIS_CLIENT["RedisTemplate / JdbcTemplate"]
        CUST_CODE["Custom Service / Batch Job"]
    end
    
    subgraph OTelLayer["OpenTelemetry Java SDK (tracer provider)"]
        TRACER[Tracer] --> SPAN_BUILDER[SpanBuilder → Span]
    end

    CONTROLLER -.->|auto-instrumented| SPAN_BUILDER
    REDIS_CLIENT -.-> |HTTP/Jedis client auto-config traces| SPAN_BUILDER
    CUST_CODE --|manual @Traced| TRACER
    
    subgraph Pipeline["Pipeline (Spring Boot)"]
        PROC[spanProcessor] --> OUT_P["OTLP gRPC/HTTP Exporter<br/>or Otel Collector pipeline"]
    end

    SPAN_BUILDER -.-> PIPELINE
```

### D4: Micrometer ↔ OpenTelemetry Bridge Layout (Spring Boot) — Flowchart

This diagram shows how Micrometer metrics and OTel traces flow from a Spring Boot app to two different observability backends. The same HTTP request gets traced by both pipelines **with proper trace propagation** between them via the `micrometer-tracing-bridge-opentelemetry` bridge dependency on the classpath.
...
graph TB
    ...
    subgraph App["Spring Boot App (4.x)"]
        HTTP["HTTP Endpoint<br/>Controller/Handler"] --> MTB["<code>micrometer-tracing-bridge-opentelemetry</code><br/><small>(auto-config)</small>"]
    end

    subgraph "Signal Exporters"
        PROM[Prometheus (/actuator/prometheus)]
        OTLP_Pipeline[OTel Collector Pipeline<br/>Zipkin / Jaeger / Grafana]
    end
    
    HTTP --> PROM["Spring Boot Actuator<br/>Micrometer metrics"]
    MTB -.->|Tracer API+SpanReporter bridge| OTLP_Pipeline

    subgraph "Observability Backends"
        G["Grafana (traces + logs)"]
        JM["Jaeger / Zipkin UI"]
        
        PMON[Prometheus Console]
        GRAFANA_METRICS["Grafana dashboards<br/>(Micrometer metrics)"]
    end

    PROM --> PMON & GRAFANA_METRICS
    OTLP_Pipeline --> G & JM
    
    note for Grafana "[Note: one request = one trace<br/>+ multiple micrometer meters]"
    
```

### D5 (E1): Micrometer Core Implementation Types — _replaced with a cleaner summary_

The relationship between `MeterRegistry` and its runtime binders is best understood through two facts rather than trying to draw the entire API in Mermaid:

| Type | What it does |
|------|--------------|
| **Interface** (`io.micrometer.core.instrument.MeterRegistry`) | Abstract base. Inject with Spring's `@Autowired MeterRegistry`. |
| **CompositeMeterRegistry** (default) | Aggregates multiple registries — by default Prometheus + another if present. The entry point for every `.counter("abc")` or `.timer()` call. |
| **PrometheusMeterRegistry** (when on classpath) | Exposes `/actuator/prometheus`. Auto-wired into CompositeMeterRegistry. |

There is no separate `MeterRegistryBinders(Gauge)` factory anywhere — instead Spring Boot's `@Bean MeterBinder` implementations (like `JvmGcMetrics`) call `registry.gauge(...)` *at startup* to register a bound point-in-time gauge that auto-updates. This pattern appears throughout the Micrometer codebase, and is what produces `jvm.gc.*`, `cache.cache.*`, etc. without custom wiring.
